...
test('runtime catalog code contains no retired provider client or synchronization route', () => {
  const runtime = [path.join(root, 'server.ts'), ...sourceFiles(path.join(root, 'routes')), ...sourceFiles(path.join(root, 'src'))]
    .map((file) => readFileSync(file, 'utf8')).join('\n');
  assert.doesNotMatch(runtime, /anilist/i);
  assert.doesNotMatch(runtime, /\/admin\/anime\/sync|catalog-images\/sync|sync\/cast/i);
});

test('ownership migration archives identifiers before removing legacy columns and state', () => {
  const migration = read('sql_scripts/025_catalog_ownership.sql');
  const archiveAt = migration.indexOf("INSERT IGNORE INTO catalog_source_refs");
  const dropAt = migration.indexOf('DROP COLUMN anilist_id');
  assert.ok(archiveAt >= 0 && dropAt > archiveAt);

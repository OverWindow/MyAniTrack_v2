                            ...
                            child: SizedBox.square(
                              ...
                              child: AppNetworkImage(
                                ...
                                cacheKey: user.profileImageUrl == null
                                    ? null
                                    : '${user.profileImageUrl}#${session.profileImageRevision}',
                                memoryBytes: session.profileImagePreview,
                                removed: session.profileImageRemoved,
                                profile: true,
                              ),
                            ),
                          ),
          ...
          child: SafeArea(
            ...
            child: ListView(
              ...
              children: [
                          ...
                          child: _preview != null
                              ...
                              : AppNetworkImage(
                                  ...
                                  cacheKey: user?.profileImageUrl == null
                                      ? null
                                      : '${user!.profileImageUrl}#${session.profileImageRevision}',
                                  memoryBytes: _removeImage
                                      ? null
                                      : (_preview ??
                                            session.profileImagePreview),
                                  removed:
                                      _removeImage ||
                                      session.profileImageRemoved,
                                  profile: true,
                                ),
                        ),
                      ),
                ...
                if ((!session.profileImageRemoved &&
                        (user?.profileImageUrl != null ||
                            session.profileImagePreview != null)) ||
                    _image != null) ...[
  ...
  Future<void> _pickImage() async {
    ...
    final cropped = await Navigator.of(context).push<Uint8List>(
      CupertinoPageRoute(
        ...
        builder: (_) => ProfileCropScreen(image: bytes),
      ),
    );
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      ref
          ...
          .applyUpdatedUser(
            updatedUser,
            profileImageChanged: _image != null || _removeImage,
            profileImagePreview: _preview,
            profileImageRemoved: _removeImage,
          );
      if (!mounted) return;
      ...
      final urls = <String>{
        ...
        if (updatedUser.profileImageUrl != null) updatedUser.profileImageUrl!,
      };
      unawaited(Future.wait(urls.map(CachedNetworkImage.evictFromCache)));
    } on ApiFailure catch (error) {
      if (mounted) showAppToast(context, error.message, error: true);
    } finally {
...
class ProfileCropScreen extends StatefulWidget {
  const ProfileCropScreen({required this.image, super.key});
...
class _ProfileCropScreenState extends State<ProfileCropScreen> {
  ...
  Rect? _currentArea;
  Size? _sourceSize;
  double? _baseAreaWidth;
  double? _baseAreaHeight;
  double _zoom = 1;

  @override
  void initState() {
    super.initState();
    final decoded = image_lib.decodeImage(widget.image);
    if (decoded != null) {
              ...
              Expanded(
                child: Padding(
                  ...
                  child: Crop(
                    ...
                    overlayBuilder: (_, _) => const ClipOval(
                      child: CustomPaint(
                        key: ValueKey('profile-crop-thirds-grid'),
                  ...
                  children: [
                    ...
                    Expanded(
                      child: CupertinoSlider(
                        key: const ValueKey('profile-crop-zoom-slider'),
                        value: _zoom,
                        min: 1,
                        max: 4,
                        divisions: 60,
                        activeColor: AppColors.point,
                        onChanged: _ready && !_cropping ? _setZoom : null,
                      ),
                    ),
                    const Icon(
  ...
  void _onCropMoved(Rect cropRect, Rect imageRect) {
    final sourceSize = _sourceSize;
    if (sourceSize == null || imageRect.width <= 0 || imageRect.height <= 0) {
      return;
    }
    final scaleX = sourceSize.width / imageRect.width;
    final scaleY = sourceSize.height / imageRect.height;
    final area = Rect.fromLTRB(
      ((cropRect.left - imageRect.left) * scaleX).clamp(0, sourceSize.width),
      ((cropRect.top - imageRect.top) * scaleY).clamp(0, sourceSize.height),
      ((cropRect.right - imageRect.left) * scaleX).clamp(0, sourceSize.width),
      ((cropRect.bottom - imageRect.top) * scaleY).clamp(0, sourceSize.height),
    );
    _currentArea = area;
    _baseAreaWidth ??= area.width;
    _baseAreaHeight ??= area.height;
    final baseWidth = _baseAreaWidth;
    if (baseWidth == null || area.width <= 0 || !mounted) return;
    final nextZoom = (baseWidth / area.width).clamp(1.0, 4.0);
  ...
  void _setZoom(double nextZoom) {
    ...
    final baseWidth = _baseAreaWidth;
    final baseHeight = _baseAreaHeight;
    if (sourceSize == null ||
        currentArea == null ||
        baseWidth == null ||
        baseHeight == null) {
    ...
    final width = baseWidth / nextZoom;
    final height = baseHeight / nextZoom;
    final center = currentArea.center;
    final left = (center.dx - width / 2)
        .clamp(0, sourceSize.width - width)
        .toDouble();
    final top = (center.dy - height / 2)
        .clamp(0, sourceSize.height - height)
        .toDouble();
    setState(() => _zoom = nextZoom);
    _controller.area = Rect.fromLTWH(left, top, width, height);
  }
            ...
            child: ListView(
              ...
              children: const [
                ...
                _LegalCard(
                  title: '데이터 출처 및 고지',
                  paragraphs: [
                    '애니메이션 정보는 공개된 자료와 검토된 출처를 기반으로 하며 저작권은 각 제공자와 권리자에게 있습니다. 분석 결과는 내 기록을 바탕으로 생성되는 참고 정보입니다.',
                  ],
                ),
              ],
            ),

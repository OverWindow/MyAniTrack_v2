...
function getShareError(error: unknown) {
  ...
  if (value.code === 'SHARE_EXPIRED' || value.status === 410) {
    return tr("이 공유 링크는 만료되었어요.")
  }
  return tr("공유 링크를 찾을 수 없거나 더 이상 사용할 수 없어요.")
}
  ...
  useEffect(() => {
    ...
    fetchShareDescriptor(token, controller.signal)
      .then((descriptor) => {
        ...
        document.title = tr("{{v0}}님의 {{v1}} | MyAniTrack", { v0: descriptor.owner.username, v1: descriptor.resourceType === 'COLLECTION' ? '컬렉션' : '분석' })
      })
      .catch((error: unknown) => {
  ...
  if (!token || (state.token === token && state.error)) {
    return (
      <section className="collection-page">
        <ConnectionErrorState message={state.token === token ? state.error ?? tr("잘못된 공유 주소예요.") : tr("잘못된 공유 주소예요.")} />
        <Link className="secondary-button" to="/">{tr("홈으로 돌아가기")}</Link>
      </section>
    )
  }

  if (state.token !== token || state.isLoading || !state.descriptor) {
    return <section className="collection-page"><div className="feedback-card">{tr("공유 화면을 불러오는 중...")}</div></section>
  }

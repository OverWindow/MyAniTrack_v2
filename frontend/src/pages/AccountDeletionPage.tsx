...
const DELETION_CONFIRMATION = tr("계정 삭제")
...
function getDeletionErrorMessage(error: unknown) {
  ...
  if (status === 401) {
    return tr("로그인 세션이 만료되었습니다. 다시 로그인한 뒤 삭제를 요청해주세요.")
  }

  if (status === 404) {
    return tr("삭제할 앱 계정을 확인하지 못했습니다. 계정 연결 상태를 확인할 수 있도록 고객지원에 문의해주세요.")
  }

  if (status && status >= 500) {
    return tr("서버에서 삭제 요청을 처리하지 못했습니다. 잠시 후 다시 시도해주세요.")
  }

  if (error instanceof TypeError) {
    return tr("네트워크 연결이 끊겨 삭제 완료 여부를 확인하지 못했습니다. 다시 로그인해 계정 상태를 확인하거나 고객지원에 문의해주세요.")
  }

  return error instanceof Error
    ? localizeExternalMessage(error.message, tr("계정 삭제에 실패했습니다. 잠시 후 다시 시도해주세요."))
    : tr("계정 삭제에 실패했습니다. 잠시 후 다시 시도해주세요.")
}
...
export function AccountDeletionPage() {
  ...
  const supportHref = `mailto:${PRIVACY_CONTACT_EMAIL}?subject=${encodeURIComponent(tr("마이애니트랙 계정 삭제 요청"))}`

  useDocumentMetadata({
    title: tr("계정 및 데이터 삭제 | 마이애니트랙"),
    description: tr("마이애니트랙 계정과 연결된 컬렉션, 분석, 프로필 및 친구 데이터를 삭제하는 방법을 안내합니다."),
  ...
  const handleCopySupportEmail = async () => {
    try {
      ...
      setCopyFeedback(tr("이메일 주소를 복사했습니다."))
    } catch {
      setCopyFeedback(tr("복사하지 못했습니다. {{v0}} 주소를 직접 복사해주세요.", { v0: PRIVACY_CONTACT_EMAIL }))
    }
  }
  ...
  return (
    <article className="policy-page deletion-page">
      <header className="policy-hero deletion-hero">
        ...
        <h1>{tr("마이애니트랙 계정 및 데이터 삭제")}</h1>
        <p>{tr("웹이나 앱을 다시 설치하지 않아도 이 페이지에서 계정 삭제를 요청할 수 있습니다.")}</p>
        <div className="policy-hero-actions">
          <Link className="secondary-button" to="/privacy">{tr("개인정보처리방침 보기")}</Link>
          <button
        ...
        {isSupportGuideOpen && (
          <div className="deletion-email-guide" id="deletion-email-guide">
            <strong>{tr("이메일 삭제 요청 방법")}</strong>
            <p>{tr("가입한 이메일 주소로 아래 문의처에 삭제 요청을 보내주세요. 계정 소유 확인을 위해 추가 확인을 요청할 수 있으며, 비밀번호나 인증 토큰은 보내지 마세요.")}</p>
            ...
            <div className="deletion-email-actions">
              ...
              <a className="primary-button" href={supportHref}>{tr("메일 앱 열기")}</a>
            </div>
      ...
      <nav className="policy-toc" aria-label={tr("계정 삭제 안내 목차")}>
        <strong>{tr("목차")}</strong>
        <ol>
          <li><a href="#deletion-data">{tr("삭제되는 데이터")}</a></li>
          <li><a href="#deletion-retention">{tr("삭제 후 보관될 수 있는 데이터")}</a></li>
          <li><a href="#deletion-request">{tr("삭제 요청")}</a></li>
        </ol>
      </nav>
      ...
      <div className="policy-content">
        <section className="policy-section" id="deletion-data">
          <h2>{tr("삭제되는 데이터")}</h2>
          <p>{tr("계정 삭제가 완료되면 다음 마이애니트랙 데이터는 복구할 수 없습니다.")}</p>
          <ul>
            <li>{tr("로그인 계정, 사용자명, 소개와 프로필 정보")}</li>
            <li>{tr("애니메이션 컬렉션, 감상 상태, 진도, 평점, 날짜와 메모")}</li>
            <li>{tr("분석 결과의 원천 기록, 최애 작품과 배지 진행 정보")}</li>
            <li>{tr("친구 관계와 주고받은 친구 요청")}</li>
            <li>{tr("이용약관과 개인정보처리방침 동의 기록")}</li>
          </ul>
        </section>

        <section className="policy-section" id="deletion-retention">
          <h2>{tr("삭제 후 보관될 수 있는 데이터")}</h2>
          <p>{tr("활성 데이터베이스의 계정 데이터는 삭제 요청 처리 과정에서 제거됩니다. 저장소의 프로필 이미지는 최대 72시간, 운영 백업은 최대 30일, 보안·장애 대응 로그는 최대 90일 동안 제한적으로 남을 수 있으며 각 기간이 지나면 파기됩니다.")}</p>
          <p>{tr("법령상 보관 의무가 발생하면 해당 근거와 기간에 따라 필요한 범위만 별도로 보관합니다.")}</p>
        </section>
        ...
        <section className="policy-section deletion-request-section" id="deletion-request">
          ...
          {deletionState === 'success' || wasDeleted ? (
            <div className="deletion-success" role="status">
              <strong>{tr("계정 삭제 요청이 완료되었습니다.")}</strong>
              <p>{tr("마이애니트랙 계정과 활성 서비스 데이터가 삭제되었습니다. 보관 정책이 적용되는 데이터는 위 기간 안에 파기됩니다.")}</p>
              <Link className="primary-button" to="/">{tr("홈으로 돌아가기")}</Link>
            </div>
          ) : isAuthenticated && user ? (
            <div className="deletion-account-card">
              <div className="deletion-account-summary">
                <span>{tr("현재 로그인 계정")}</span>
                <strong>{user.username || tr("사용자명 미설정")}</strong>
                <small>{maskEmail(user.email)}</small>
              </div>
              <div className="deletion-warning" role="note">
                <strong>{tr("이 작업은 되돌릴 수 없습니다.")}</strong>
                <p>{tr("다른 기기의 로그인 세션도 더 이상 사용할 수 없으며, 삭제한 컬렉션과 분석은 복구되지 않습니다.")}</p>
              </div>
              <label className="policy-confirm-field" htmlFor="account-deletion-confirmation">
                <span>{tr("계속하려면 “")}{DELETION_CONFIRMATION}{tr("”를 입력하세요.")}</span>
                <input
              ...
              {errorStatus === 401 && (
                <Link to="/login" state={{ from: '/account-deletion' }}>{tr("다시 로그인하기")}</Link>
              )}
            </div>
          ) : (
            <div className="deletion-signed-out-card">
              <strong>{tr("로그인할 수 있다면 웹에서 바로 삭제할 수 있습니다.")}</strong>
              <p>{tr("로그인 후 확인 문구와 최종 확인 단계를 거쳐 계정과 연결 데이터를 삭제합니다.")}</p>
              <Link className="primary-button" to="/login" state={{ from: '/account-deletion' }}>
                {tr("로그인하여 계정 삭제")}
              </Link>
              <hr />
              <strong>{tr("로그인할 수 없나요?")}</strong>
              <p>{tr("가입한 이메일 주소에서 고객지원으로 삭제 요청을 보내주세요. 계정 소유 확인 후 처리하며, 이메일에 비밀번호나 인증 토큰을 적지 마세요.")}</p>
      ...
      <footer className="policy-footer">
        ...
        <a href={supportHref}>{tr("삭제 관련 문의")}</a>
      ...
      {deletionState === 'confirming' && (
        <div className="policy-modal-backdrop" role="presentation" onMouseDown={() => setDeletionState('idle')}>
          ...
          >
            ...
            <h2 id="account-deletion-dialog-title">{tr("정말 계정을 삭제할까요?")}</h2>
            <p>{tr("컬렉션, 평점, 분석, 프로필과 친구 관계가 삭제되고 다른 기기의 세션도 사용할 수 없습니다.")}</p>
            <div className="policy-modal-actions">
              <button className="secondary-button" type="button" onClick={() => setDeletionState('idle')}>{tr("취소")}</button>
